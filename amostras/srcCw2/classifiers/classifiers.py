...
def trainTestSplit(labels, characteristics):
    labelEncoder = preprocessing.LabelEncoder()
    
    encodedLabels=labelEncoder.fit_transform(labels)
    decodedLabels = labelEncoder.inverse_transform(encodedLabels)
    #for i in range(0,len(encodedLabels)):
    #    print('label:'+str(labels[i])+', encodedLabel:'+str(encodedLabels[i])+', decodedLabel:'+str(decodedLabels[i]))

    features=characteristics  
    
    tmpYtest = {}
    tmpYtrain = {}
    X_train, X_test, y_train, y_test = train_test_split(np.array(features), encodedLabels, test_size=0.5, random_state=42)
    for y in y_test:
        if y not in tmpYtest:
            tmpYtest[y] = 0
        tmpYtest[y] += 1
    for y in y_train:
        if y not in tmpYtrain:
            tmpYtrain[y] = 0
        tmpYtrain[y]+=1
    return [X_train, X_test, y_train, y_test]

'''
def plotConfusionMatrix(classifier, X_test, y_test, labelsMap):
    class_names = []
    for i in range(0,len(labelsMap.keys())):        
        class_names.append(labelsMap[str(i)])
...
def mountResultsTrainTest(model,labels, characteristics):
    ...
    #plotConfusionMatrix(model, X_test, y_test, map)
