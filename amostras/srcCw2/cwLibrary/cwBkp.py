...
def to_tanh_space(x, box):
    ...
    output = atanh((x - _box_plus) / _box_mul)
...
class L2Adversary(object):
    ...
    def _optimize(self, model, optimizer, inputs_tanh_var, pert_tanh_var,targets_oh_var, c_var, index):
        ...
        #
        # noinspection PyArgumentList
        #print("pert_outputs_var 2.0")
        #print(pert_outputs_var)
        #print("pert_outputs_var.max(1)")
        #print(pert_outputs_var.max(1))
        #print("pert_outputs_var.max(1)[0]")
        #print(pert_outputs_var.max(1)[0])
        #print('(pert_outputs_var.max(1)[0] >= -inf)')
        #print((pert_outputs_var.max(1)[0] >= -inf))
        #print('(pert_outputs_var.max(1)[0] >= -inf).all()')
        #print((pert_outputs_var.max(1)[0] >= -inf).all())
