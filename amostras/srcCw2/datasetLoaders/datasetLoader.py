...
class TonetDataSet():
    ...
    def filterClasses(self,arffLoadedData):
        ...
        outputDataset = []
        minInstances = len(self.dataSetsName) * 200
        ...
        for i in range(0,len(arffLoadedData)):           
            ...
            if className not in classesHashMap:
                continue
            if len(classesHashMap[className])<=minInstances:
    ...
    def preProcessDataset(self):
        ...
        print(filteredData['classesHashMap'].keys())
        if self.normalize:
            torch.save(torch.tensor(filteredData['database']),'../outputs/datasets/datasetLoader-tensorDatabase.pt')
            filteredData['database'] = self.normalizeData(filteredData['database'])
            torch.save(torch.tensor(filteredData['database']),'../outputs/datasets/datasetLoader-normalized-tensorDatabase.pt')
        else:
            torch.save(torch.tensor(filteredData['database']),'../outputs/datasets/datasetLoader-tensorDatabase.pt')
