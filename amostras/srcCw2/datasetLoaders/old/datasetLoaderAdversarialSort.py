...
class AdversarialDataSet():
    ...
    def __init__(self, adversarialDatasetPath,adversarialLabelsPath, labelsPercentage):
        self.labels = None
        self.labelsStr = None
        self.labelsHashMap = {}
    ...
    def sortByNumericName(self, files):
        numericFiles = []
        for f in files:
            if '.txt' in f or 'adversarial_examples_' not in f:
                continue
            tmp = f.replace('.pt','')
    ...
    def loadAdversarialFiles(self,totalData,labels):
        datasets = self.sortByNumericName(os.listdir(self.adversarialDatasetPath))    
        numberOfConsideredFiles = int((len(datasets))*(self.labelsPercentage/100))   
    ...
    def preProcessDataset(self):
        ...
        self.loadAdversarialFiles(totalData,labels) 
        print('loaded')
        #if self.normalize:   
        ...
        print('Porcentagem de amostras reais utilizado: 0%')
        
        
        return self.splitIntoTrainTestValidation({'database':totalData,'labels':labels})
